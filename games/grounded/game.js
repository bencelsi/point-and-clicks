...
const startData = { title: 'Grounded', room: 'room', frame: 'A1', extension: 'jpeg', 
    customCursors: false, frameWidth: 640, frameHeight: 480 } 
...
const roomData = {
    'room': {
        ...
        'A1a': { back: 'A1' },
        'A2': { left: 'A1', right: 'A3', boxes: [{ xy: [.57, .8, .5, .83], to: 'A2a' }, { xy: [.34, .4, .36, .5], to: 'A1a'}]},
        'A2a': { back: 'A2', boxes: [{ xy: [.28, .34, .89, .97], to: 'A2b', fn: () => {playSound('creak')} }] },
        'A2b': { back: 'A2', boxes: [{ xy: [.28, .34, .89, .97], to: 'A2a', fn: () => {playSound('creak')} }, 
        ...
        'A2c': { back: 'A2b', boxes: [
            { xy: [.32, .37, .53, .61], fn: () => { if (comboMatches([0, 0, 0, 0])) { playSound('safe'); wait(1, () => { goTo( s.pig == 1 ? 'A2d' : 'A2e') })}}},
        ...
        'A11': { left: 'A10', right: 'A1', boxes: [
            { xy: [.4, .55, .4, .6], to: () => { return s.lightOn ? 'A11b' : 'A11a' }},
        ...
        'B1': { left: 'B2', right: 'B2', back: 'A10',
            boxes: [{ xy: [.61, .68, .45, .62], id: 'doorknob', cursor: () => { return s.key == 2 ? 'forward' : null },
