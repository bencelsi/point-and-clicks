...
const roomData = {
    'room': {
        'M': { forward: () => { playSound('music/heartz.mp3'); wait(0, () => { goTo('A1') }) }},
        ...
        'A2c': { back: 'A2b', boxes: [
            { xy: [.32, .37, .53, .61], cursor: 'O', fn: () => {
                if (comboMatches([0, 0, 0, 0])) { playSound('safe'); 
                    for (i in [1, 2, 3, 4]) wait(i * .5, () => { playSound('comboShort')})
                    
                    wait(2, () => { goTo( s.pig == 1 ? 'A2d' : 'A2e') })}}},
            { xy: [.41, .46, .53, .63], cursor: 'O', fn: () => { buttonPush([0, 1]); refresh() }}, 
            { xy: [.46, .51, .53, .63], cursor: 'O', fn: () => { buttonPush([0, 1, 2]); refresh() }}, 
            { xy: [.51, .56, .53, .63], cursor: 'O', fn: () => { buttonPush([1, 2, 3]); refresh() }}, 
            { xy: [.56, .61, .53, .63], cursor: 'O', fn: () => { buttonPush([2, 3]); refresh() }},
...
const inventory = {
    ...
    pig: { img: 'pig', targets: [{ frame: 'A6a', fn: () => { 
        playSound('smash'); s.pig = 2; refreshInventory(); goTo('A6b'); wait(.25, ()=> { goTo('A6c') })}}]},
}
