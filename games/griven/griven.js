...
// WASD keys
// cache boxes
// Save menu, extras etc
// No gifs
// Make it possible to drag small key without zooming in, just show zoom shot for a moment
// wait for toilet to refill before flushing
// Compress all images
// Fix elevator num framing
// fade time bug
// pic flicker
// Cursor - tidy edges
// Cursor option
// Higher quality brochure
...
'clockroom' : {
    ...
    'A2': { left: 'A1', right: 'A3', boxes: [{ xy: [.29, .34, .42, .48], if: () => { return !s.clockOn }, 
        ...
        else { freeze(); await d(4.5); s.clockOn = false; refreshBoxes(); unfreeze() }}},
    ...
    'A5': { back: async () => { s.jesusCount++; 
        if (s.jesusCount == 3) {
            freeze(); setMusic(null); playSound('jesus'); await d(2)
            goTo('A2a');
...
'top': {
    ...
    'D3': { left: 'D2', right: { to: 'D4', to: () => { if (s.otherLeft) { playSound('bobb/otherLeft'); s.otherLeft = false }}}, 
        boxes: [
            { xy: [.57, .61, .44, .49], fn: async () => {
                if (!s.bobbSpeech) { playSound('doorLocked'); return }
                freeze(); setMusic(null); playSound('music/end'); playSound('bobb/jump'); 
                hideInventory(); playGif('exit6', 'D6'); await d(7); setFade(7); goTo('E1'); await d(5)
...
async function pushKeypad(n) {
    ...
    if (room == 'pool' && comboIs([3, 5, 2, 9, 9])) { playSound('doorOpen'); goTo('A3a'); s.clockUnlocked = true }
        ...
        await d(60); s.officeUnlocked = true; goTo('C2a'); playSound('doorOpen'); unfreeze() }}
