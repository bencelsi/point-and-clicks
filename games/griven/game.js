...
const elevatorBoxes = [
    ...
    { pic: () => { return 'scratch/floor' + s.floor }, offset: [.467, .663], scale: '8', if: () => { return s.floor > 1 && s.floor < 10}}
]
...
const gameData = {
    ...
    frames: {
        ...
        'cafe': { //zcafe
            ...
            'A7': { back: () => { return s.plumbingDoorOpen ? 'A1a' : 'A1' }},
        ...
        'lobby' : { //zlobby
            ...
            'D3': { left: 'D2', right: 'D1', boxes: [
                { fn: () => { playSound('bell') }, xy: [.49, .56, .33, .4]},
                ...
                { pic: 'D3-rightLightKey', if: () => { return s.lightsOn && !s.smallKey == 0 }},
                { pic: 'D3-rightLightKey', if: () => { return s.lightsOn && !s.smallKey == 0 }},
            ...
            'H4': { left: 'H3', right: 'H1', boxes: [
                { to: () => { playSound('doorOpen.wav'); return 'H4a'}, xy: [.55, .7, .4, .63] }]},
        ...
        'stairs': { //zstairs
            ...
            'B5a':{ back: () => { playSound('drawer'); return'B1' }, boxes: [
                { pic: 'B5a-card', if: () => { return s.coffeeNote }},
                { pic: 'B5a-note', if: () => { return s.coffeeNote }},
                { pic: 'B5a-coffee', if: () => { return s.coffeeNote }}
            ]},
            'C1': {}
        },
...
// Saveable State
const s = {
    smallKey: 0, // 0 - on chair, 1- in inventory, 2- in drawer
    lightsOn: false,
    cabinetDown: false,
    lightsOn: false,
    drawers: [false, true, false, true],
    valves: [false],
    hallPosition: 0,
    hallDirection: 0,
    floor: 1,
...
function pushKeypad(n) {
    ...
    if (room == 'pool') { checkCombo([3, 5, 2, 9, 9], 'A2a') } 
    else if (room == 'cafe') { checkCombo([8, 7, 0, 1, 2], 'A1a') }
    else { checkCombo([1, 2, 6, 6, 9], 'C2a') }
}

function checkCombo(desired, destination) {
    for (i in desired) { if (s.combo[i] != desired[i]) { return } }
    playSound('doorOpen.wav')
    transitionTo(destination, 'fade')
}
