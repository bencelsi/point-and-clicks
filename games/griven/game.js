...
// Add 'final' music 
// TODO: options - classic cursor / screech transitions / new music
...
const gameData = {
    ...
    rooms: {
        ...
        'cafe': { //zcafe
            ...
            'A7': { back: () => { return s.plumbingDoorOpen ? 'A1a' : 'A1' }, boxes: [
                ...
                { xy: [.34, .4, .46, .52], fn: () => { playSound('button')
                    if (s.heaterLevel >= 0 && !s.valves[2] && s.valves[4] && s.pipe == 3) { playSound('slurp'); alert('JOE') }
                    else { makeEphemeralBox('scratch/noWater', 1000) }}}]},
        ...
        'elevator': { //zelevator
            ...
            'A3': { forward: () => { playSound('panel'); return 'A3a' }, back: () => { return s.floor === 1? 'A2a' : (s.floor === 10 ? 'A2c' : 'A2b') }},
            'A3a': { back: () => { playSound('panel'); return s.floor === 1? 'A2a' : ('A2' + s.floor === 10 ? 'c' : 'b') }, boxes: [
                ...
                boxes: [
                    { xy: [.5, .72, .2, .35], fn: () => {  makeEphemeralBox('doorHandle2', 1000); 
                        if (s.floor == 2 && s.hallPosition == 7 && s.hallDirection == 1) {
                            playSound('doorOpen'); transitionTo('A5a' , 'fade') } else { playSound('doorHandle');}}}],
                back: 'A5' },
            'B1': { left: 'B4', right: 'B2', boxes: [ outerElevatorBox,
                { to: () => { playSound('elevatorOpen');
        ...
        'lobby' : { //zlobby
            ...
            'C1': { left: 'C4', right: 'C2', boxes: [ outerElevatorBox,
                { to: () => { playSound('elevatorOpen'); return s.elevatorFloor === 1 ? 'C1b' : 'C1a' }, 
                    xy: [.28, .31, .48, .52] }]},
            ...
            'D3': { left: 'D2', right: 'D1', boxes: [
                ...
                { xy: () => { return s.drawers[0] ? [.3, .37, .38, .48] : [.22, .3, .38, .48] }, 
                    fn: () => { if (s.smallKey == 2) { playSound('cabinet' + (s.drawers[0] ? 'Close' : 'Open')); changeDrawer(0) }
                        else { transitionTo('D4', 'fade') }}, if: () => { return !s.cabinetDown }},
            ...
            'E4': { back: 'E2', boxes: [
                { pic: 'scratch/clockHand1', offset: [.485, .81] },
                { pic: 'scratch/clockHand2', offset: [.48, .73] }]},
            'F1': { left: 'F2', right: 'F2',
                forward: () => { s.floor = 2; 
                    playSound('stairsUp');
        ...
        'pool': { //zpool
            ...
            'A2a':{ left: () => { playSound('doorClose'); return 'A1'}, 
                    right: () => { playSound('doorClose'); return 'A3'}, boxes: [
                        { to: 'B2', xy: [.54, .65, .3, .51] }]},
            ...
            'B1': { left: 'B4', right: 'B2' },
            'B2': { left: 'B1', right: 'B3', 
                forward: () => { playGif('ladderUp', 'clockroom/A3', 10 * 150) }},
            'B3': { left: 'B2', right: 'B4' },
...
function checkCombo(desired, destination) {
    ...
    playSound('doorOpen'); transitionTo(destination, 'fade'); return true
}
